...
public class Lexer {
	...
	private final int MAX_STRING = 1024; // max size of a string, used to avoid
											// infinite loops if some delimiters
											// are omitted
	final int BUFFER_MARK = 1000; // use .mark and .reset of buffer to move it
									// back and forth
	BufferedReader file;
	...
	// This method is where the lexical analyzr reads the file char by char and
	// creates the tokens
	...
	public Token nextToken() throws IOException {
		...
		if (Token.isDel(val)) {
			...
			// if the double quotation is found then we know that it is a string
			// literal
			if (val == '"') {
				counter = 0;
				temp += val;
				...
				// append each char to temp until we get to the closing double
				// quotes
				// also limit to the max string size incase there is a missing
				// ".
				while (val != '"' && counter < MAX_STRING) {
					counter++;
					temp += val;
					val = (char) file.read();
				}

				// if the next val is not closing double quotes then string was
				// missing quote, sset as error token
				if (val != '"') {
					curr.setStringErrorToken();
					return curr;
				}
			...
			return curr;
		} else if (Token.isOp(val)) // need to check if its a negative number or
									// a double symbol op(!=, >=, etc)
		{

			// check for negative numbers
			if (val == '-') {
				// if the next character is a nummber we can assume that it is a
				// negative int or float
				if (nextChar() == 4) {
					temp += val;

					val = (char) file.read();
					file.mark(BUFFER_MARK); // mark current spot in buffer
					...
					// if there is a period next to the digits we assume that it
			...
			// if the next character is an operator, we check to see if its a
			// logical operator that has 2 chars
		...
		// creates a token for float or integer if a digit is found
			...
			// if a period is directly after a number assume that this number is
			// a float
			if (val == '.') {
				temp += val;
				val = (char) file.read();
		...
		// if the char read is a letter we know it must either be a keyword or a
		// usermade identifier
	...
	public void outputTokens(String outputFile) throws IOException {
		...
		for (int i = 0; i < tokens.size() - 1; i++) {
			if (tokens.get(i).lexeme.equals("")) {
				// I cant figure out why it keeps adding some blank lexemes so
